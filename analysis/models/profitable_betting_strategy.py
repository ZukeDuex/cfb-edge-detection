#!/usr/bin/env python3
"""Profitable Betting Strategy Model integrating ML predictions with betting lines."""
...
def main():
    print("💰 Profitable Betting Strategy Model")
    print("=" * 50)
    ...
    # 6. Integrate betting lines and create profitable strategy
    print(f"\n💰 Creating profitable betting strategy...")
    create_profitable_betting_strategy(engineered_df, odds_df, ml_model)
    
    # Save comprehensive data
    filename = 'tennessee_games_profitable_betting.csv'
    engineered_df.to_csv(filename, index=False)
    print(f"💾 Profitable betting data saved to: {filename}")
...
def create_profitable_betting_strategy(df, odds_df, ml_model):
    """Create profitable betting strategy integrating ML predictions with betting lines."""
    ...
    if total_bets > 0:
        ...
        medium_edge_bets = [b for b in profitable_bets if 3 < abs(b['ml_edge']) <= 5]
        ...
        if medium_edge_bets:
            ...
            print(f"   Medium Edge (3-5): {len(medium_edge_bets)} | Profit: ${medium_edge_profit:+.2f} | Win Rate: {medium_edge_win_rate:.1%}")
