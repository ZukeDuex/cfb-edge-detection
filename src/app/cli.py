...
def ingest(
    ...
    weather: Optional[str] = typer.Option(
        None, help="Weather provider (meteostat or weatherapi)"
    ),
):
    ...
    try:
        ...
        if weather:
            if weather.lower() == "weatherapi":
                provider_instance = WeatherAPIProvider()
            elif weather.lower() == "meteostat":
                provider_instance = MeteostatWeatherProvider()
            else:
                console.print(
                    f"[bold red]Unknown weather provider: {weather}[/bold red]"
                )
                raise typer.Exit(1)

        ingest_pipeline = IngestPipeline(weather_provider=provider_instance)
        results = ingest_pipeline.run_full_ingest(
            season=season, week=week, include_weather=weather is not None
        )
