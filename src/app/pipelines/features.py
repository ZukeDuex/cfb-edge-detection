    ...
    def calculate_rolling_features(self, team_stats_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for team in team_stats_df["team"].unique():
            ...
            for window in self.rolling_windows:
                # Rolling EPA
                team_data[f"epa_off_{window}g"] = (
                    team_data["epa_off"].rolling(window=window, min_periods=1).mean()
                )
                team_data[f"epa_def_{window}g"] = (
                    team_data["epa_def"].rolling(window=window, min_periods=1).mean()
                )

                # Rolling success rate
                team_data[f"success_rate_off_{window}g"] = (
                    team_data["success_rate_off"]
                    .rolling(window=window, min_periods=1)
                    .mean()
                )
                team_data[f"success_rate_def_{window}g"] = (
                    team_data["success_rate_def"]
                    .rolling(window=window, min_periods=1)
                    .mean()
                )

                # Rolling pace
                team_data[f"pace_{window}g"] = (
                    team_data["pace"].rolling(window=window, min_periods=1).mean()
                )

                # Rolling explosiveness
                team_data[f"explosiveness_off_{window}g"] = (
                    team_data["explosiveness_off"]
                    .rolling(window=window, min_periods=1)
                    .mean()
                )
                team_data[f"explosiveness_def_{window}g"] = (
                    team_data["explosiveness_def"]
                    .rolling(window=window, min_periods=1)
                    .mean()
                )

                # Rolling finishing drives
                team_data[f"finishing_drives_off_{window}g"] = (
                    team_data["finishing_drives_off"]
                    .rolling(window=window, min_periods=1)
                    .mean()
                )
                team_data[f"finishing_drives_def_{window}g"] = (
                    team_data["finishing_drives_def"]
                    .rolling(window=window, min_periods=1)
                    .mean()
                )
