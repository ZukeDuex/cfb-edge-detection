...
class FeaturePipeline:
    ...
    def calculate_epa_features(self, games_df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("Calculizing EPA features")
        ...
        features_df['success_rate_off'] = features_df.get('offensive_yards_per_play', 0) / 6.0
        features_df['success_rate_def'] = 1 - (features_df.get('defensive_yards_per_play', 0) / 6.0)
        
        # Pace (plays per game)
        features_df['pace'] = features_df.get('offensive_plays', 0) + features_df.get('defensive_plays', 0)
        
        # Explosiveness (simplified)
        features_df['explosiveness_off'] = features_df.get('offensive_yards_per_play', 0) * 0.2
        features_df['explosiveness_def'] = features_df.get('defensive_yards_per_play', 0) * -0.2
        
        # Finishing drives (simplified)
        features_df['finishing_drives_off'] = features_df.get('offensive_points_per_game', 0) / 10.0
        features_df['finishing_drives_def'] = 1 - (features_df.get('defensive_points_per_game', 0) / 10.0)
    ...
    def calculate_rolling_features(self, team_stats_df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Calculated rolling features for {len(result_df)} team-game records")
        return result_df
    ...
    def create_feature_matrix(self, games_df: pd.DataFrame, team_stats_df: pd.DataFrame) -> pd.DataFrame:
        ...
        required_features = [
            'epa_off_3g', 'epa_def_3g', 'epa_off_5g', 'epa_def_5g', 'epa_off_10g', 'epa_def_10g',
            'success_rate_off_3g', 'success_rate_def_3g', 'success_rate_off_5g', 'success_rate_def_5g',
            'pace_3g', 'pace_5g', 'line_movement', 'movement_velocity', 'cross_book_delta',
            'temp_c', 'wind_mps', 'precip_mm', 'days_rest', 'travel_distance'
        ]
        ...
        logger.info(f"Created feature matrix with {len(features_df)} records and {len(features_df.columns)} features")
        return features_df
    ...
    def run_feature_engineering(self, as_of_date: str) -> pd.DataFrame:
        ...
        team_stats_df = pd.read_parquet(team_stats_files[0]) if team_stats_files else pd.DataFrame()
