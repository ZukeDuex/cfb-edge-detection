#!/usr/bin/env python3
"""Enhanced ML model incorporating comprehensive CFBD API stats."""
...
def main():
    ...
    # 3. Game stats
    print(f"   🏈 Fetching game stats...")
    game_stats = fetch_game_stats(api_key, headers, tennessee_df)
    ...
    enhanced_df = create_comprehensive_dataset(tennessee_df, team_stats, player_stats, game_stats, advanced_stats)
    
    # 6. Build enhanced ML model
...
def fetch_game_stats(api_key, headers, tennessee_df):
    """Fetch game stats."""
    ...
    try:
        for _, game in tennessee_df.iterrows():
            game_id = game['id']
            print(f"      🏈 Fetching game stats for {game_id}...", end=" ")
            ...
            if response.status_code == 200:
                ...
                if data and len(data) > 0:
                    game_stats.append({
                        ...
                        'home_stats': data[0] if len(data) > 0 else {},
                        'away_stats': data[1] if len(data) > 1 else {}
                    })
                    print(f"✅")
                else:
                    print(f"⚠️  No stats")
...
def create_comprehensive_dataset(tennessee_df, team_stats, player_stats, game_stats, advanced_stats):
    ...
    for _, game in tennessee_df.iterrows():
        ...
        enhanced_record['ppg_difference'] = enhanced_record['tennessee_ppg'] - enhanced_record['opponent_ppg']
        enhanced_record['papg_difference'] = enhanced_record['opponent_papg'] - enhanced_record['tennessee_papg']
        enhanced_record['ypg_difference'] = enhanced_record['tennessee_ypg'] - enhanced_record['opponent_ypg']
        enhanced_record['yapg_difference'] = enhanced_record['opponent_yapg'] - enhanced_record['tennessee_yapg']
        enhanced_record['turnover_difference'] = enhanced_record['tennessee_turnovers_forced'] - enhanced_record['opponent_turnovers']
        enhanced_record['efficiency_difference'] = enhanced_record['tennessee_overall_eff'] - enhanced_record['opponent_overall_eff']
