#!/usr/bin/env python3
"""Retrieve CFBD stats for opponents to enhance ML predictions."""
...
def main():
    ...
    # Get unique opponents and seasons
    opponents_data = []
    ...
    for data in opponents_data:
        ...
        if season not in unique_opponents:
            unique_opponents[season] = set()
        unique_opponents[season].add(opponent)
    
    print(f"\n📈 Found opponents by season:")
    for season, opponents in unique_opponents.items():
    ...
    for season, opponents in unique_opponents.items():
        ...
        for opponent in opponents:
            print(f"   📊 {opponent} ({season})...")
            
            try:
                # Fetch team stats for the season
                stats = fetch_team_stats(api_key, headers, opponent, season)
...
def fetch_team_stats(api_key, headers, team, season):
    """Fetch comprehensive team stats for a given team and season."""
...
def analyze_enhanced_features(enhanced_df):
    ...
    # Calculate Tennessee performance by opponent stats
    ...
    # Analyze by turnover margin
    print(f"\n🔄 Tennessee Performance by Opponent Turnover Margin:")
    
    enhanced_df['opponent_turnover_tier'] = pd.cut(
        enhanced_df['opponent_turnover_margin'], 
        bins=[-10, -0.5, 0.5, 10], 
        labels=['Negative', 'Neutral', 'Positive']
    )
    
    turnover_performance = enhanced_df.groupby('opponent_turnover_tier').agg({
        'tennessee_won': ['count', 'sum', 'mean'],
        'tennessee_point_differential': 'mean'
    })
    
    for tier, stats in turnover_performance.iterrows():
        if pd.notna(tier):
            wins = stats[('tennessee_won', 'sum')]
            total = stats[('tennessee_won', 'count')]
            win_pct = stats[('tennessee_won', 'mean')]
            ...
            print(f"   vs {tier} turnover margin: {wins}/{total} ({win_pct:.1%}) | Avg diff: {avg_diff:.1f}")
